...
if (isset($_GET['id']))
{
	...
	if (!$existing_record) {
		print  "<p style='max-width:700px;margin-top:15px;color:#800000;'>
	...
	print	RCView::table(array('style'=>'width:700px;table-layout:fixed;','class'=>'hidden-xs'),
			RCView::tr('',
						...
						RCView::table(array('id'=>'status-icon-legend'),
							...
							RCView::tr('',
								RCView::td(array('class'=>'nowrap', 'style'=>'padding-right:5px;'),
									RCView::img(array('src'=>'circle_red.gif')) . $lang['global_92']
								) .
							...
							RCView::tr('',
								...
								RCView::td(array('class'=>'nowrap', 'style'=>''),
									(!$surveys_enabled ? "" :
										RCView::img(array('src'=>'tick_circle_frame.png')) . $lang['global_94']
									)
								)
							)
						)
	...
	{
		if ($_GET['msg'] == 'edit') {
			print "<div class='darkgreen' style='margin:10px 0;width:640px;'><img src='".APP_PATH_IMAGES."tick.png'> ".RCView::escape($table_pk_label)." <b>{$_GET['id']}</b> {$lang['data_entry_08']}</div>";
		} elseif ($_GET['msg'] == 'add') {
			print "<div class='darkgreen' style='margin:10px 0;width:640px;'><img src='".APP_PATH_IMAGES."tick.png'> ".RCView::escape($table_pk_label)." <b>{$_GET['id']}</b> {$lang['data_entry_09']}</div>";
		} elseif ($_GET['msg'] == 'cancel') {
			print "<div class='red' style='margin:10px 0;width:640px;'><img src='".APP_PATH_IMAGES."exclamation.png'> ".RCView::escape($table_pk_label)." <b>{$_GET['id']}</b> {$lang['data_entry_11']}</div>";
	...
	***************************************************************/
	...
	foreach ($Proj->forms as $form_name=>$attr)
	{
		...
		if ($prev_form != $row['form_name'])
		{
			...
			print "<tr><td class='data' style='line-height:18px;'>".RCView::escape($row['form_menu_description']);
		...
		foreach ($form_events as $this_event_id=>$eattr)
		{
			...
			foreach ($grid_form_status[$row['event_id']][$row['form_name']] as $this_instance=>$this_form_status) {
				...
				if (in_array($row['form_name'], $eattr))
				{
					...
					$this_button = "<img src='".APP_PATH_IMAGES."$this_color' style='height:16px;width:16px;'>";
...
else
{
	...
	***************************************************************/
	...
	if ($auto_inc_set && $user_rights['record_create'] > 0)// && $num_records <= $maxNumRecordsHideDropdowns)
	{
